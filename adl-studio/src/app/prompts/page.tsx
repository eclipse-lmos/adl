...
function PromptEditorPageContent() {
  ...
  const availableTags = useMemo(() => tagsResult.data?.tags || [], [tagsResult.data]);
  ...
  const handlePromptSave = useCallback(() => {
    ...
    executeStoreMutation({
        ...
        if (error) {
            toast({
                variant: 'destructive',
                title: 'Error storing Skill Prompt',
  ...
  const handleContractChange = async (isChecked: boolean) => {
    ...
    try {
      ...
      if (result.error) {
        setEditedTestCase(editedTestCase);
        toast({
          variant: 'destructive',
  ...
  return (
    <div className="flex min-h-screen w-full flex-col bg-background">
      ...
      <main className="flex flex-1 flex-col overflow-hidden px-4 pb-4 md:px-6 md:pb-6">
        <div className="flex h-12 shrink-0 items-center justify-between border-b px-2 py-1 mb-4">
          ...
          <div className="flex items-center gap-4">
            <TagManager tags={tags} onTagsChange={setTags} availableTags={availableTags} />
            <Select
      ...
      <VersionHistory
        ...
        useCaseId={useCaseId}
        onLoadVersion={(version: VersionEntry) => {
          setPrompt(version.content);
          setTags(version.tags || []);
          setUtterances(version.examples || []);
          toast({ title: `Loaded version ${version.version}`, description: 'Save to apply changes.' });
        }}
      />

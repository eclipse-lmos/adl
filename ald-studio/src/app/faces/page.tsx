...
function FacesContent() {
    ...
    useEffect(() => {
        ...
        const interactionsFromUrl = searchParams.get('interactions');
        if (nameFromUrl) {
            setName(nameFromUrl);
        }
        if (purposeFromUrl) {
        ...
        if (interactionsFromUrl) {
            setInteractions(interactionsFromUrl);
        }
                    ...
                    <AccordionContent className="p-4 pt-0">
                        <form onSubmit={handleGenerateWidget} className="w-full max-w-4xl mx-auto space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div className="space-y-2">
                                    <Label htmlFor="widget-interactions">Interactions</Label>
                                    <Input 
                                        id="widget-interactions"
                                        placeholder="e.g. button clicks, form submissions"
                                        value={interactions}
                                    ...
                                    <Card className="flex-1">
                                        <CardContent className="h-full p-0">
                                            <Textarea
                                                value={jsonSchema}
                                                readOnly
                                                className="h-full w-full resize-none rounded-lg bg-muted p-4 text-sm font-mono border-0 focus-visible:ring-0 focus-visible:ring-offset-0"
                                            />
                                        </CardContent>
                                    </Card>
                                </div>
...
export default function FacesPage() {
    return (
        <Suspense fallback={<div className="flex h-screen w-full items-center justify-center"><Loader2 className="h-4 w-4 animate-spin" /></div>}>
            <FacesContent />
        </Suspense>
    );
}

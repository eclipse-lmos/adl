...
import { Bot, User, Trash2, FileText, Wand2, ShieldCheck } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export default function ConversationMessage({
  ...
  const otherContext = message.context?.filter(c => c.key !== 'useCase' && c.key !== 'useCaseContent' && c.key !== 'compliance');
  ...
  return (
    <div className='flex items-start gap-4'>
      ...
      <div className="flex-1 space-y-2">
        ...
        <div className="rounded-lg border p-4 text-sm bg-card">
            {isEditing ? (
                <Textarea
        ...
        {message.role === 'assistant' && !isEditing && (useCaseContentContext || complianceContext) && (
                    ...
                    return (
                       <Collapsible key={i} className="text-left">
                           <CollapsibleTrigger className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground w-full justify-start cursor-pointer p-1 disabled:cursor-default" disabled={!isLong}>
                                <span className={cn(badgeVariants({ variant: 'secondary' }), "max-w-full")}>

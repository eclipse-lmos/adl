...
const PromptHighlighter = {
  rules: {
    VALID_SECTION: {
      test: (line: string) =>
        /^\s*##+\s*(?:UseCase:|Description|Solution|Alternative Solution|Steps)/.test(
          line
        ),
  ...
  highlight(line: string, inCodeBlock: boolean, isCursorOnLine?: boolean): (string | JSX.Element)[] | JSX.Element {
    if (inCodeBlock) {
      return <span className="font-code text-muted-foreground">{line}</span>;
    }
    ...
    if (this.rules.VALID_SECTION.test(line)) {
      ...
      const parts = line.split(/(<.*?>|@[a-zA-Z0-9\-_]+|```\w*)/g);
...
export default function PromptEditor({ prompt, onPromptChange, onSave, isDirty, isSaving, onBlur, tools = [] }: PromptEditorProps) {
  ...
  const handleSuggestionSelect = (suggestion: string) => {
    ...
    if (suggestionType === 'section') {
        ...
        if (match) {
            ...
            // Fallback, should not happen if autocomplete is visible
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    // Defer update to get correct cursor position after browser handles event
  ...
  const handleTextChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    const calculatePositionAndShow = (suggestions: string[], type: 'section' | 'tool') => {
        if (suggestions.length > 0) {
            ...
            const textarea = textareaRef.current;
            if (textarea) {
              // Create a mirror div to calculate the position of the caret
              ...
              // The container of the textarea is the `relative` parent
              const container = textarea.parentElement;
              if (container) {
                container.appendChild(mirrorDiv);

                // Set content up to the cursor position
                mirrorDiv.textContent = textarea.value.substring(0, cursorPosition);

                // Create a span at the end of the text to get its coordinates
                const span = document.createElement('span');
                // The span needs some content to have dimensions
                span.textContent = '.';
                mirrorDiv.appendChild(span);

                // Calculate position relative to the container, and adjust for scroll
    ...
    if (textBeforeCursor.endsWith('##')) {
      const allSuggestions = ["UseCase", "Description", "Solution", "Alternative Solution", "Steps"];
        ...
        <CardHeader className="p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="text-lg font-semibold">UseCase Prompt</CardTitle>
          ...
          <div className="relative h-full">
            ...
            <Popover open={showAutocomplete} onOpenChange={setShowAutocomplete}>
              <Textarea
                ...
                className="relative w-full h-full resize-none font-mono text-sm rounded-lg bg-transparent text-transparent caret-foreground"
              />
      ...
      <Dialog open={isFullScreen} onOpenChange={setIsFullScreen}>
        <DialogContent className="sm:max-w-4xl h-[80vh] flex flex-col p-0">
          <DialogHeader className="p-6 pb-4 border-b">
            <div className="flex items-center justify-between">
              <DialogTitle>UseCase Prompt</DialogTitle>
              <TooltipProvider delayDuration={0}>
                <Tooltip>
                    <TooltipTrigger asChild>
                    <Button
          ...
          <div className="relative flex-1 overflow-hidden">
            ...
            <Textarea
                ...
                placeholder="Enter your prompt here..."
                className="relative w-full h-full resize-none font-mono text-sm bg-transparent text-transparent caret-foreground p-0 border-0 focus-visible:ring-0 focus-visible:ring-offset-0 px-6 py-4"
                spellCheck="false"
            />
          </div>
        </DialogContent>
